...
fn sub_64_bit_integers_render_as_json_number() {
    let cases: &[(FieldType, Vec<u8>, Value)] = &[
        ...
        (
            FieldType::U32,
            to_vec(&4_000_000_000u32).unwrap(),
        ...
        (
            FieldType::I32,
            to_vec(&-2_000_000_000i32).unwrap(),
...
fn nested_struct_roundtrip() {
    ...
    let ty = FieldType::Struct(vec![
        ...
        NamedField {
            name: "inner".into(),
            ty: FieldType::Struct(vec![NamedField {
                name: "amount".into(),
                ty: FieldType::U64,
            }]),
        },
    ...
    assert_eq!(
        got,
        json!({ "label": "stake", "inner": { "amount": "500000" } })
    );
}
...
fn enum_struct_variant() {
    ...
    let bytes = to_vec(&E::Active {
        user: 42,
        score: 9_999_999_999_u64,
    })
    .unwrap();
    let ty = FieldType::Enum(vec![
        EnumVariant {
    ...
    assert_eq!(
        got,
        json!({ "Active": { "user": 42, "score": "9999999999" } })
    );
}
...
fn defined_type_resolves_via_registry() {
    ...
    defined.insert(
        ...
        FieldType::Struct(vec![NamedField {
            name: "value".into(),
            ty: FieldType::U64,
        }]),
    );
